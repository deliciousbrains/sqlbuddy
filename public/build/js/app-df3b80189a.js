...
},{"./bootstrap":5}],5:[function(require,module,exports){
'use strict';
...
},{"./components/sidebar":6,"./components/sqlbuddy":7,"./components/table":8,"./util/pagination":9,"vue":3,"vue-resource":2}],6:[function(require,module,exports){
'use strict';

Vue.component('sqlbuddy-sidebar', {
	props: ['databases', 'selectedDatabase', 'selectedTable'],
	...
	methods: {
		getTables: function getTables(database) {
			var _this = this;

			this.$http.get('/api/databases/' + encodeURI(database) + '/tables').then(function (response) {
				if (response.data.error) {
					_this.$dispatch('error', response.data.error);
				} else {
	...
	watch: {
		'selectedDatabase': function selectedDatabase(database) {
			if (database) {
...
Vue.component('sqlbuddy', {
	...
	methods: {
		getDatabases: function getDatabases() {
			...
			this.$http.get('/api/databases').then(function (response) {
				_this.databases = response.data;
			});
		}
	},
...
Vue.component('sqlbuddy-table', {
	props: ['selectedDatabase', 'selectedTable'],
	...
	methods: {
		getTableRows: function getTableRows(table, page) {
			var _this = this;
			...
			this.$http.get('/api/databases/' + encodeURI(this.selectedDatabase) + '/tables/' + encodeURI(table) + '?page=' + page).then(function (response) {
				if (response.data.error) {
					_this.$dispatch('error', response.data.error);
				} else {
	...
	watch: {
		'selectedTable': function selectedTable(table) {
			this.getTableRows(table);
		}
	},

	events: {
		'pagination-rows-page': function paginationRowsPage(page) {
...
Vue.component('pagination', {
	...
	template: '<ul class="pagination" v-if="data.total > data.per_page">' + '	<li class="page-item" v-if="data.prev_page_url">' + '		<a href="#" class="page-link" aria-label="Previous" @click.prevent="selectPage(--this.data.current_page)"><span aria-hidden="true">&laquo;</span><span class="sr-only">Previous</span></a>' + '	</li>' + '	<li class="page-item" v-for="n in data.last_page" :class="{ \'active\': n + 1 == data.current_page }"><a href="#" class="page-link" @click.prevent="selectPage(n + 1)">{{ n + 1 }}</a></li>' + '	<li class="page-item" v-if="data.next_page_url">' + '		<a href="#" class="page-link" aria-label="Next" @click.prevent="selectPage(++this.data.current_page)"><span aria-hidden="true">&raquo;</span><span class="sr-only">Next</span></a>' + '	</li>' + '</ul>',
